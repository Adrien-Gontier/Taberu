...
import NutriscorPictC from '../assets/pictures/nutriscore-c.svg'
import NutriscorPictD from '../assets/pictures/nutriscore-d.svg'
...
export default function OneFavorites(props) {
    ...
    useEffect(() => {
        favorites.find((oneProduct) => oneProduct.code == oneFavorites.code)
    ...
    const favoritesNoneIcon = (
        <img
            onClick={() => {
                ...
                favoritesCopy.push(oneFavorites)
    ...
    const favoritesPresentIcon = (
        <img
            onClick={() => {
                setBooleanFavorites(false)
                let favoritesCopy = favorites
                const indexId = favoritesCopy.indexOf(oneFavorites.code)
    ...
    const brands = (
        <p className="favorites__name">{oneFavorites.product.brands}</p>
    )

    const picture = (
        <div className="favorites__DivTag">
            <img
                className="favorites__DivTag__productPict"
                src={oneFavorites.product.image_url}
                alt="image produit"
            />
        </div>
    )
    ...
    const caloriesUnit = (
        <>{oneFavorites.product.nutriments['energy-kcal_unit']}</>
    )
    const caloriesPer100g = (
        <p className="favorites__caloriesPer100">
    ...
    const caloriesDataPresent = oneFavorites.product.nutriments[
        'energy-kcal'
    ] ? (
        caloriesPer100g
    ) : (
    ...
    return (
        <>
            <div className="favorites__oneProduct">
                {brands}
                {favoritesButton}
            </div>
            <>{dataProduct}</>
