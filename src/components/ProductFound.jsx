...
export default function ProductFound() {
    ...
    useEffect(() => {
        favorites.find(oneProduct => oneProduct.code == productResult.code) ? setBooleanFavorites(true) : setBooleanFavorites(false);
    }, []);

    const favoritesNoneIcon = <img onClick={() => { setBooleanFavorites(!booleanFavorites); let favoritesCopy = favorites; favoritesCopy.push(productResult); setFavorites(favoritesCopy); }} src={FavoritesEmptyIcon} alt="" />;
    const favoritesPresentIcon = <img onClick={() => { setBooleanFavorites(false); let favoritesCopy = favorites; const indexId = favoritesCopy.indexOf(productResult.code); favoritesCopy.splice(indexId, 1); setFavorites(favoritesCopy); }} src={FavoritesFullIcon} alt="" />;
    ...
    const brands = <p className="product__name">{productResult.product.brands}</p>;

    const picture = <img className="productPict" src={productResult.product.image_url} alt="image produit" />;

    const calories = <>{productResult.product.nutriments["energy-kcal"]}</>;
    const caloriesUnit = <>{productResult.product.nutriments["energy-kcal_unit"]}</>;
    const caloriesPer100g = <p className="product__caloriesPer100">{calories} {caloriesUnit} /100g </p>;
    ...
    const caloriesDataPresent = productResult.product.nutriments["energy-kcal"] ? caloriesPer100g : <p className="product__caloriesPer100">Information sur les calories non disponibles</p>

    const dataProduct = <>{brands}{picture}{caloriesDataPresent}{nutriscorePict}</>;
