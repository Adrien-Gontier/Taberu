...
export default function NameNumberSearch() {
    ...
    const glassIcon = <img className="nameNumberSearch__input-glassIcon__GlassIcon" src={GlassIcon} alt="Glass icon" />;
    ...
    useEffect(() => {
        ...
        API_URL = "https://world.openfoodfacts.org/api/v0/product/" + barCodeNumber + ".json";
    ...
    return (
        <div className="nameNumberSearch">
            ...
            <div className="nameNumberSearch__input-glassIcon">
                <label>
                    <input value={nameNumberFilled} onChange={(event) => { setNameNumberFilled(event.target.value); }} type='text/number' />
                </label>
                {nameNumberFilled == null ? glassIcon : <Link onClick={() => setBarCodeNumber(nameNumberFilled)}>{glassIcon}</Link>}
            </div>
        </div>
    )
}
